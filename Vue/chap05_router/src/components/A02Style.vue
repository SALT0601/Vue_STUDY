<template>
  <div v-cloak>
    ...
    <div v-bind:style="{ color: 'orange', 'font-size': '24pt' }">
      {{ hello }}
    </div>
    <!-- 문자로 묶거나 fontSize와 같은 카멜 표기법 이용 -->
    <div v-bind:style="{ color: 'orange', fontSize: '24pt' }">{{ hello }}</div>
    <div
      v-bind:style="[styleOne, styleTwo]"
    ...
    <div class="one" v-bind:class="{ two: true, three: isChecked }">
      {{ hello }}
    </div>
    <div class="one" v-bind:class="[classTwo, { three: isChecked }]">
      {{ hello }}
    </div>
